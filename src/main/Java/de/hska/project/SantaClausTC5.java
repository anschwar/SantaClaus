...
public class SantaClausTC5 {
    ...
    // problem dimensions
    private final static int NUMBER_OF_REINDEER = 36;
    private final static int NUMBER_OF_ELVES = 40;
    private final static int ELVES_NEEDED_TO_WAKE_SANTA = 3;
    private final static int REINDEERS_NEEDED_TO_WAKE_SANTA = 9;

    // synchronisation variables
    private final Semaphore queueElves;
    private final Semaphore queueReindeers;
        ...
        public void run() {
            while (kidsStillBelieveInSanta) {
                try {
                    ...
                    // only 9 at the same time. Rest has to wait
                    queueReindeers.acquire();
    ...
    public SantaClausTC5() {
        ...
        threeElves = new CyclicBarrier(ELVES_NEEDED_TO_WAKE_SANTA, new NoOpRunnable());
        elvesAreInspired = new CyclicBarrier(ELVES_NEEDED_TO_WAKE_SANTA, new NoOpRunnable());
        queueReindeers = new Semaphore(REINDEERS_NEEDED_TO_WAKE_SANTA, true);
        allReindeers = new CyclicBarrier(REINDEERS_NEEDED_TO_WAKE_SANTA, new NoOpRunnable());
